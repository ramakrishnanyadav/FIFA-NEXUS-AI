...
Metrics generated by executing the load-testing harness ([benchmark_load.py](../backend/tests/benchmark_load.py)) locally against a running backend API:
...
Key architectural choices are documented in the [docs/adr/](adr/) folder:

1.  **[ADR-001: FastAPI for API Gateway](adr/ADR-001-fastapi.md)** — Core asynchronous gateway choice.
2.  **[ADR-002: LightGBM for Predictor](adr/ADR-002-lightgbm.md)** — Gradient boosting choice for predictive forecasting.
3.  **[ADR-003: Qdrant Vector Engine](adr/ADR-003-qdrant.md)** — Semantic SOP vector matching.
4.  **[ADR-004: API Key Authentication](adr/ADR-004-api-key.md)** — Service-level access controls.
5.  **[ADR-005: Graceful Offline Fallbacks](adr/ADR-005-fallback-strategy.md)** — Zero-LLM/Offline resiliency loops.
6.  **[ADR-006: Deferred Alembic Migrations](adr/ADR-006-alembic-migrations.md)** — Dynamic SQLAlchemy model reflection for MVP stages.
