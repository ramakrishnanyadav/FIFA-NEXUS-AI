...
from backend.app.schemas.schemas import TelemetryCreate, OperationalEventCreate
...
async def test_telemetry_threshold_breach():
    ...
    # Setup database query execution returns
    result_mock = MagicMock()
    result_mock.scalars().first.return_value = zone_mock
...
def test_constraint_optimization():
    # Test action ranking and safety filtering
    actions = [
    ...
    # Redirect to Gate A should be filtered out due to safety warning
    assert len(result["actions"]) == 2
    assert "Redirect fans to Gate A (High density path)" not in result["actions"]
    assert "WARN_HIGH_DENSITY_REDIRECTION" in result["policy_flags"]
